...
// MeteringEventHandler 处理计量事件
type MeteringEventHandler struct{}

func (h *MeteringEventHandler) Handle(message *EventStreamMessage) ([]SSEEvent, error) {
	var data map[string]any
	if err := utils.FastUnmarshal(message.Payload, &data); err != nil {
		return nil, err
	}
...
// ContextUsageEventHandler 处理上下文使用事件
...
func (h *ContextUsageEventHandler) Handle(message *EventStreamMessage) ([]SSEEvent, error) {
	...
	// 提取 contextUsagePercentage 字段
	percent := 0.0
	if p, ok := data["contextUsagePercentage"].(float64); ok {
		percent = p
	}
