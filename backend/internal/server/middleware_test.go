...
func createTestKeyManager(token string) *auth.APIKeyManager {
	keyMgr := auth.NewAPIKeyManager(nil) // 测试不需要数据库
	keyMgr.AddKey(auth.APIKeyConfig{
		Key:  token,
		Name: "test",
	})
	return keyMgr
...
func TestPathBasedAuthMiddleware_ValidToken(t *testing.T) {
	...
	// 配置中间件
	keyMgr := createTestKeyManager("test-token-123")
	protectedPrefixes := []string{"/v1/"}

	router.Use(PathBasedAuthMiddleware(keyMgr, protectedPrefixes))
