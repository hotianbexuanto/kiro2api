...
func (c *UsageLimitsChecker) CheckUsageLimits(token types.TokenInfo) (*types.UsageLimits, error) {
	...
	// 设置请求头 (使用统一的伪装常量 + 动态指纹)
	tokenID := token.RefreshToken
	if len(tokenID) > 16 {
		tokenID = tokenID[:16]
	}
	fingerprint := config.GetFingerprintManager().GenerateFingerprint(tokenID)
	kiroUA := fmt.Sprintf("KiroIDE-%s-%s", config.KiroIDEVersion, fingerprint)
	req.Header.Set("x-amz-user-agent", fmt.Sprintf("aws-sdk-js/%s %s", config.KiroSDKVersion, kiroUA))
	req.Header.Set("user-agent", fmt.Sprintf("aws-sdk-js/%s ua/2.1 os/%s lang/js md/nodejs#%s api/codewhispererruntime#%s m/E %s",
		config.KiroSDKVersion, config.KiroOS, config.KiroNodeVersion, config.KiroSDKVersion, kiroUA))
